	...
	return {
		...
		load(id) {
			...
			try {
				...
				const generatedParser = generate(grammar, Object.assign({ output: "source" }, options));
				
				// Fix naming conflict with global SyntaxError and CommonJS requires
				let fixedParser = generatedParser.replace(/class peg\$SyntaxError extends SyntaxError/g, 'class peg$SyntaxError extends globalThis.SyntaxError');
				
				// Convert CommonJS require to import for ES modules
				fixedParser = fixedParser.replace(/const { Vector } = require\('\.\/types'\);/g, "import { Vector } from './types';");
