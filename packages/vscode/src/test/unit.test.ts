...
test("Error message formatting", () => {
  ...
  expect(formatError({ message: "Custom error" })).toBe(
    "Custom error (line 1, column 1)",
  );
  expect(
    formatError({
      message: "Parse error",
...
test("Content validation", () => {
  ...
  expect(isValidLatexContent("\\begin{document}Hello\\end{document}")).toBe(
    true,
  );
  ...
  function createValidRange(
    ...
    return {
      start: {
        ...
        character: Math.max(0, startColumn),
      },
      end: {
        line: Math.max(startLine, endLine),
        character: Math.max(0, endColumn),
      },
    };
  }
