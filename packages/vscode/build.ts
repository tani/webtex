...
async function copyWebTexFiles() {
  ...
  if (!existsSync(coreDistPath)) {
    throw new Error("Core package dist directory not found. Please build the core package first with 'bun run compile:core'");
  }
...
async function buildExtension(options: BuildOptions = {}) {
  ...
  console.log(`🔨 Building VSCode extension${production ? ' (production)' : ''}${watch ? ' (watch mode)' : ''}...`);
  ...
  async function buildOnce() {
    ...
    if (result.success) {
      ...
      console.log(`✅ Build completed successfully in ${duration}ms (${outputCount} files)`);
      ...
      if (result.logs.length > 0) {
        result.logs.forEach((log: any) => console.error(log.message));
      }
      process.exit(1);
    }
  }
...
function parseArgs(): BuildOptions {
  ...
  return {
    ...
    production: args.includes("--production") || args.includes("--prod") || args.includes("-p")
  };
}
