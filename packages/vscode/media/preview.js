...
function compileAndRender(latex) {
  const api = window?.__WebTeX ? window.__WebTeX : null;
  if (!api) {
  ...
  try {
    ...
    const payload = {
      type: "compileError",
      error: {
        message: err?.message ? String(err.message) : "WebTeX conversion error",
        location: err?.location ? err.location : undefined,
      },
    };
