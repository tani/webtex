...
export class LaTeX {
  ...
  private setupLengths(): void {
    const lengths = [
      '@@size', 'unitlength', '@wholewidth', 'paperheight', 'paperwidth',
      'oddsidemargin', 'evensidemargin', 'textheight', 'textwidth',
      'marginparwidth', 'marginparsep', 'marginparpush', 'columnwidth',
      'columnsep', 'columnseprule', 'linewidth', 'leftmargin', 'rightmargin',
      'listparindent', 'itemindent', 'labelwidth', 'labelsep',
      'leftmargini', 'leftmarginii', 'leftmarginiii', 'leftmarginiv',
      'leftmarginv', 'leftmarginvi', 'fboxrule', 'fboxsep', 'tabbingsep',
      'arraycolsep', 'tabcolsep', 'arrayrulewidth', 'doublerulesep',
      'footnotesep', 'topmargin', 'headheight', 'headsep', 'footskip',
      'topsep', 'partopsep', 'itemsep', 'parsep', 'floatsep',
      'textfloatsep', 'intextsep', 'dblfloatsep', 'dbltextfloatsep'
    ];
  ...
  private setupArgs(): void {
    ...
    // Font commands
    const fontCommands = ['rm', 'sf', 'tt', 'md', 'bf', 'up', 'it', 'sl', 'sc', 'normal'];
    ...
    // Font size commands
    const fontSizes = ['tiny', 'scriptsize', 'footnotesize', 'small', 'normalsize', 'large', 'Large', 'LARGE', 'huge', 'Huge'];
    ...
    // Box commands
    args['llap'] = args['rlap'] = args['clap'] = args['smash'] = 
    args['hphantom'] = args['vphantom'] = args['phantom'] = ['H', 'hg'];
    args['underline'] = ['H', 'hg'];
    args['mbox'] = ['H', 'hg'];
    ...
    args['alph'] = args['Alph'] = args['arabic'] = 
    args['roman'] = args['Roman'] = args['fnsymbol'] = ['H', 'i'];
    ...
    args['samepage'] = args['newpage'] = args['clearpage'] = 
    args['cleardoublepage'] = args['vfill'] = ['HV'];
  ...
  // Font size commands
  public tiny(): void { this.g.setFontSize("tiny"); }
  public scriptsize(): void { this.g.setFontSize("scriptsize"); }
  public footnotesize(): void { this.g.setFontSize("footnotesize"); }
  public small(): void { this.g.setFontSize("small"); }
  public normalsize(): void { this.g.setFontSize("normalsize"); }
  public large(): void { this.g.setFontSize("large"); }
  public Large(): void { this.g.setFontSize("Large"); }
  public LARGE(): void { this.g.setFontSize("LARGE"); }
  public huge(): void { this.g.setFontSize("huge"); }
  public Huge(): void { this.g.setFontSize("Huge"); }
  ...
  public itemize(items?: any[]): any {
    ...
    return [this.g.create(this.g.unorderedList, items!.map(item => {
      this.g.enterGroup();
      const makelabel = this.g.create(this.g.itemlabel, this.llap(item.label !== null
        ? item.label
        : this.g.macro(label)));
      this.g.exitGroup();
  ...
  public parbox(pos?: string, height?: any, innerPos?: string, width?: any, txt?: any): any[] {
    if (!pos) pos = "c";
    ...
    switch (pos) {
      case "c": classes += " p-c"; break;
      case "t": classes += " p-t"; break;
      case "b": classes += " p-b"; break;
      default: this.g.error("unknown position: " + pos);
    }

    switch (innerPos) {
      case "s": classes += " stretch"; break;
      case "c": classes += " p-cc"; break;
      case "t": classes += " p-ct"; break;
      case "b": classes += " p-cb"; break;
      default: this.g.error("unknown inner-pos: " + innerPos);
    }
  ...
  private _path(p: string, N?: number): any {
    ...
    draw.size(this.g.round(bbox.width) + "px", this.g.round(bbox.height) + "px")
        .viewbox(this.g.round(bbox.x), this.g.round(bbox.y), this.g.round(bbox.width), this.g.round(bbox.height));
    draw.flip('y', 0);
  ...
  public _line(vs: any, ve: any): any {
    ...
    const draw = this.g.SVG().addTo(svg);
    const linethickness = this.g.length('@wholewidth');
    const bbox = draw.line(vs.x.px, vs.y.px, ve.x.px, ve.y.px)
                     .stroke({ color: "#000", width: linethickness.value })
                     .bbox();
  ...
  public _vector(vs: any, ve: any): any {
    ...
    const bbox = draw.line(s.x.px, s.y.px, ve.x.px, ve.y.px)
                     .stroke({ color: "#000", width: linethickness.value })
                     .marker('end', hl, hw, (marker: any) => {
                       return marker.path("M0,0 Q" + this.g.round(2 * hl / 3) + "," + this.g.round(hw / 2) + 
                                         " " + hl + "," + this.g.round(hw / 2) + 
                                         " Q" + this.g.round(2 * hl / 3) + "," + this.g.round(hw / 2) + 
                                         " 0," + hw + " z");
                     })
                     .bbox();
  ...
  public oval(maxrad?: any, size?: any, part?: string): any[] {
    ...
    const draw = this.g.SVG();
    const oval = draw.rect(size.x.value, size.y.value)
                    .radius(rad.value)
    ...
    const clip = draw.clip().add(draw.rect(rect.w.value, rect.h.value).move(rect.x.value, rect.y.value));
    clip.flip('y', 0);
    oval.clipWith(clip);
  ...
  // File commands
  public input(file: any): void {}
  public include(file: any): void {}

  // Document class command
  public documentclass(options?: any, documentclass?: string, version?: string): void {
  ...
  // Package command
  public usepackage(opts?: any, packages?: string[], version?: string): void {
  ...
  public includeonly(filelist: string[]): void {}
  public makeatletter(): void {}
  public makeatother(): void {}

  // Page style commands
  public pagestyle(s: string): void {}

  // Line break commands
  public linebreak(o?: number): void {}
  public nolinebreak(o?: number): void {}
  public fussy(): void {}
  public sloppy(): void {}

  // Page break commands
  public pagebreak(o?: number): void {}
  public nopagebreak(o?: number): void {}
  public samepage(): void {}
  public enlargethispage(s: boolean, l: any): void {}
  public newpage(): void {}
  public clearpage(): void {}
  public cleardoublepage(): void {}
  public vfill(): void {}
  public thispagestyle(s: string): void {}
}