...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('webtex-preview.openPreview', (uri?: vscode.Uri) => {
            WebTeXPreviewPanel.createOrShow(context.extensionUri, uri);
        }),
        vscode.commands.registerCommand('webtex-preview.openPreviewToSide', (uri?: vscode.Uri) => {
            WebTeXPreviewPanel.createOrShow(context.extensionUri, uri, vscode.ViewColumn.Beside);
        })
    );
...
export async function getLatexContent(currentUri: vscode.Uri | undefined): Promise<string> {
    if (!currentUri) {
        return '';
    }
    try {
        const openDoc = vscode.workspace.textDocuments.find(d => d.uri.toString() === currentUri.toString());
        if (openDoc) {
...
class WebTeXPreviewPanel {
    ...
    public static createOrShow(extensionUri: vscode.Uri, uri?: vscode.Uri, column?: vscode.ViewColumn) {
        ...
        WebTeXPreviewPanel.currentPanel = new WebTeXPreviewPanel(panel, extensionUri, uri);
    }
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, uri?: vscode.Uri) {
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (msg?.type === 'compileError') {
                ...
                const message: string = typeof error.message === 'string' && error.message.length
                    ? error.message
                    : 'WebTeX conversion error';

                const startLine1 = Number(error?.location?.start?.line) || 1;
                const endLine1 = Number(error?.location?.end?.line) || startLine1;
                ...
                const range = new vscode.Range(
                    ...
                    new vscode.Position(lineIndex, Math.max(endCol1 - 1, lineText.length))
                );
        ...
        this._disposables.push(
            vscode.window.onDidChangeTextEditorSelection((evt) => {
                ...
                if (evt.textEditor.document.uri.toString() !== this._currentUri.toString()) {
                    return;
                }
                const line = evt.selections?.[0]?.active?.line ?? 0;
                // VS Code is 0-based; data-source-line is 1-based
                this._panel.webview.postMessage({ type: 'scrollToLine', line: (line + 1) });
            })
        );
    }
    ...
    private async _getHtmlForWebview(webview: vscode.Webview) {
        ...
        const webtexBaseCssPath = vscode.Uri.joinPath(this._extensionUri, 'node_modules', 'webtex', 'dist', 'css', 'base.css');
        const webtexBaseCssUri = webview.asWebviewUri(webtexBaseCssPath);
        const webtexArticleCssPath = vscode.Uri.joinPath(this._extensionUri, 'node_modules', 'webtex', 'dist', 'css', 'article.css');
        const webtexArticleCssUri = webview.asWebviewUri(webtexArticleCssPath);
        const webtexBrowserPath = vscode.Uri.joinPath(this._extensionUri, 'node_modules', 'webtex', 'dist', 'webtex.browser.js');
        ...
        const scriptPath = vscode.Uri.joinPath(this._extensionUri, 'media', 'preview.js');
...
function isTexDocument(doc: vscode.TextDocument): boolean {
    return doc.languageId === 'latex' || doc.fileName.toLowerCase().endsWith('.tex');
}
